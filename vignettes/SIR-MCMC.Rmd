...
This example uses the Eyam data, which consists the population of Susceptible, Infected, and Removed groups at several time points.
...
Here, we choose $\mathcal{N}(0, 100^2)$ as the prior for both $\log \alpha$ and $\log \beta$.
...
We will use the random walk Metropolis algorithm implemented in the function *MCMCmetrop1R()* (**MCMCpack** package) to explore the posterior distribution. So, we first need to install the pacakge and its dependencies.
...
```{r longMCMCrun, cache=TRUE, cache.lazy=FALSE}
...
We can visualize the joint posterior distribution of $\log \alpha$ and $\log \beta$ using **ggplot2** package.
...
We can also construct the $95\%$ credible interval for $\alpha$ and $\beta$.
