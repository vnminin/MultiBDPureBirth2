...
// [[Rcpp::export]]
void phi_Cpp (const std::complex<double> s, const int a0, const int b0, 
    const std::vector<double>& lambda2, const std::vector<double>& mu2,  // TODO Not used
  ...
  std::vector<std::complex<double>> yvec(dim + maxdepth), lentz(dim), inv_Bk1dBk(dim), BidBj(dimsq);
      ...
      for (int a=0; a<=(A-a0); ++a) {        
        ...
        BidBj = BidBj_Cpp(B,xvec,yvec,inv_Bk1dBk); // TODO Compute in place
        ...
        for (int i=0; i<=B; ++i) 
            ...
            // TODO prod_mu2, prod_lambda2 as flat vector
            // TODO flat iterator over triangular space                    
		    }
  }
}
