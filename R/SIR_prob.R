...
#'   beta  <- exp(param[2])
#'   if(length(unique(rowSums(data[, c("S", "I", "R")])))> 1) stop ("Please make sure the data conform with a closed population")
#'
...
#'                  )[data$S[k] - data$S[k + 1] + 1, data$R[k + 1] - data$R[k] + 1] # To: R(t_(k+1)), I(t_(k+1))
#'                )
#'              }))
#' }
#' 
