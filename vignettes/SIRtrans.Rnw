...
nSim = 5000 #number of MC simulations
...
<<transitionCompare>>=
library(plotrix)
inds <- t(which(tpmArray[5,2,,] >= sort(tpmArray[5,2,,], decreasing=T)[16], arr.ind=TRUE))
#ind1 <- sample(52,25, replace=T); ind2 <- sample(25,25,replace=T)
par(mfrow = c(4,4), oma = c(5,4,0,0) + 0.1, mar = c(0,0,1,1) + 0.1)
for(i in 1:16){
  plot(tList, tpmArray[,2,inds[1,i], inds[2,i] ], pch = 3, col = 'red', ylim = c(0,.006),
     yaxt = 'n', xlab = "dt")
  MCp <- tpmArray[,3,inds[1,i], inds[2,i] ] #MC prob
  plotCI(tList, MCp, pch = 16, col = 'purple', ui=MCp+1.96*sqrt(MCp*(1-MCp)), 
         li=MCp-1.96*sqrt(MCp*(1-MCp)), add = TRUE)
  points(tList, tpmArray[,1,inds[1,i], inds[2,i] ], col='green')
}
